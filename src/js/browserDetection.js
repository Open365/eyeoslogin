...
define(function () {
    ...
    browserDetector.prototype.detectMobile = function() {
        ...
        if (isMobile && document.cookie.indexOf("desktopVersion") === -1) {
    ...
    browserDetector.prototype.detectBrowser = function() {
        var BrowserSupport = {};

        BrowserSupport.Opera = (!!window.opr && !!opr.addons) || !!window.opera || navigator.userAgent.indexOf(' OPR/') >= 0;
        BrowserSupport.Firefox = typeof InstallTrigger !== 'undefined';
        BrowserSupport.Safari = Object.prototype.toString.call(window.HTMLElement).indexOf('Constructor') > 0;
        BrowserSupport.IE = false || !!document.documentMode;
        BrowserSupport.Edge = !BrowserSupport.IE && !!window.StyleMedia;
        ...
        if (BrowserSupport.Chrome || BrowserSupport.Firefox || BrowserSupport.Opera) {
            ...
            for (var i = 0; i < tokens.length; i++) {
                if (tokens[i].substr(0, 6) === 'Chrome') {
                    version = tokens[i].split('/')[1];
                    version = version.split('.')[0];
                    browser = 'Chrome';
                    break;
                }
                if (tokens[i].substr(0, 7) === 'Firefox') {
                    version = tokens[i].split('/')[1];
                    version = version.split('.')[0];
                    browser = 'Firefox';
                    break;
            ...
            if (version < window.platformSettings.minBrowsersVersion[browser]) {
                location.replace("./browserNotSupported.html");
            }
        }
    };
